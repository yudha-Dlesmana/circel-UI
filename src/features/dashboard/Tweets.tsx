...
import { useTweets } from "@/features/dashboard/useTweet";
...
export function Tweets(){
  ...
  const {TweetsPayload, isLoading, error} = useTweets()

  if(isLoading) return <h1> Loading ... </h1>
  if(error) return <h1> Error loading Feeds </h1>
  if( (TweetsPayload?.tweets ?? 0) === 0 ) return <h1>No Feeds</h1>

  return(
    <>
    {TweetsPayload?.tweets.map((tweet) => 
